...
export class ModelFeatures {
    ...
    end_ints;
    locked;
    constructor(beg_size, mid_size, end_size, padding_token, use_inputs_at_offsets) {
        if (mid_size != 0) {
            throw new Error(`Assertion failed: This implementation does not support mid_size (${mid_size}) != 0 model config.`);
        }
        if (use_inputs_at_offsets) {
            throw new Error(`Assertion failed: This implementation does not support use_inputs_at_offsets = true model config.`);
        }
