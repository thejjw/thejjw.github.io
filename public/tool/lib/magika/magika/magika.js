...
export class Magika {
    ...
    _getResultFromFewBytes(fileBytes, path = "-") {
        ...
        try {
            ...
            return this._getResultFromLabelsAndScore(path, Status.OK, ContentTypeLabel.UNDEFINED, ContentTypeLabel.TXT, 1.0);
        }
        catch (error) {
            return this._getResultFromLabelsAndScore(path, Status.OK, ContentTypeLabel.UNDEFINED, ContentTypeLabel.UNKNOWN, 1.0);
        }
    }
    static _lstrip(fileBytes) {
        let startIndex = 0;
    ...
    async _identifyFromBytes(fileBytes) {
        if (fileBytes.length === 0) {
            return this._getResultFromLabelsAndScore("-", Status.OK, ContentTypeLabel.UNDEFINED, ContentTypeLabel.EMPTY, 1.0);
        }
        if (fileBytes.length < this.model_config.min_file_size_for_dl) {
            return this._getResultFromFewBytes(fileBytes);
        }
        const features = Magika._extractFeaturesFromBytes(fileBytes, this.model_config.beg_size, this.model_config.mid_size, this.model_config.end_size, this.model_config.padding_token, this.model_config.block_size, this.model_config.use_inputs_at_offsets);
        return await this._getResultFromFeatures(features);
    }
    _getOutputLabelFromModelPrediction(model_prediction) {
        let overwrite_reason = OverwriteReason.NONE;
        // Overwrite model_prediction.label if specified in the overwrite_map.
        let output_label = this.model_config.overwrite_map[model_prediction.label] ??
    ...
    static _extractFeaturesFromBytes(fileBytes, beg_size, mid_size, end_size, padding_token, block_size, use_inputs_at_offsets) {
        const begChunk = this._lstrip(fileBytes.slice(0, Math.min(block_size, fileBytes.length)));
        const begBytes = begChunk.slice(0, Math.min(begChunk.length, beg_size));
        const endChunk = this._rstrip(fileBytes.slice(Math.max(0, fileBytes.length - block_size)));
        const endBytes = endChunk.slice(Math.max(0, endChunk.length - end_size));
        const endOffset = Math.max(0, end_size - endBytes.length);
        return new ModelFeatures(beg_size, mid_size, end_size, padding_token, use_inputs_at_offsets)
            .withStart(begBytes, 0)
            .withEnd(endBytes, endOffset);
    }
    _getContentTypeInfo(label) {
        return this.cts_infos[label];
    }
    _getResultFromLabelsAndScore(path, status = Status.OK, dl_label, output, score, overwrite_reason = OverwriteReason.NONE, scores_map) {
        return {
            path: path,
            status: status,
            prediction: {
    ...
    async _getResultFromFeatures(features) {
        let model_prediction = await this.model.predict(features);
        let [output_label, overwrite_reason] = this._getOutputLabelFromModelPrediction(model_prediction);
        return this._getResultFromLabelsAndScore("-", Status.OK, model_prediction.label, output_label, model_prediction.score, overwrite_reason, model_prediction.scores_map);
    }
    _getModelName(pathOrUrl) {
