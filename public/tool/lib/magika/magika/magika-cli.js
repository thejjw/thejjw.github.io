...
program
    ...
    .option("--model-config-url <model-config-url>", "Model config URL", Magika.MODEL_CONFIG_URL)
...
(async () => {
    ...
    await Promise.all(program.args.map(async (path) => {
        ...
        if (data != null) {
            if (flags.byStream) {
                const magika_result_by_stream = await magika.identifyStream(fs.createReadStream(path), data.length);
                if (flags.jsonOutput) {
                    console.log(path, magika_result_by_stream);
                }
                else if (flags.debug) {
                    console.log(chalk.yellow(path), "by_stream", chalk.green(magika_result_by_stream.prediction.dl.label, magika_result_by_stream.prediction.output.label), chalk.white(magika_result_by_stream.prediction.score));
                }
                else {
                    console.log(chalk.yellow(path), chalk.green(magika_result_by_stream.prediction.output.label), chalk.white(magika_result_by_stream.prediction.score.toFixed(3)));
                }
            }
            else {
                ...
                if (flags.jsonOutput) {
                    ...
                    console.log(chalk.yellow(path), "by_path", chalk.green(magika_result_by_path.prediction.dl.label, magika_result_by_path.prediction.output.label), chalk.white(magika_result_by_path.prediction.score));
                }
                else {
                    console.log(chalk.yellow(path), chalk.green(magika_result_by_path.prediction.output.label), chalk.white(magika_result_by_path.prediction.score.toFixed(3)));
                }
            }
        }
    }));
