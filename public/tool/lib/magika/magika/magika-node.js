...
export class MagikaNode extends Magika {
    ...
    async load(options) {
        ...
        if (options?.modelConfigPath != null) {
            ...
            promises.push(this.model_config.loadUrl(options?.modelConfigURL || Magika.MODEL_CONFIG_URL));
        }
     ...
     * prediction.
     */
    async identifyStream(stream, length) {
        let result = await this._identifyFromStream(stream, length);
        return result;
    }
    async _identifyFromStream(stream, length) {
        ...
        stream.on("data", (data) => {
            ...
            if (length <= 4 * this.model_config.block_size) {
                ...
                if (fileData.length > 0) {
                    ...
                    // "end block".
                    accData = accData.subarray(accData.length - this.model_config.block_size);
